...
public class TextLayout {
    ...
    private Point wrapSize = new Point();
    private final int[][] lines = new int[10240][4]; //160KB of buffer allow 10240 lines

    public TextLayout(Editable text, Paint paint, TextDrawer drawer) {
        this.paint = paint;
        this.text = text;
    ...
    public void measure(int width, int height) {
        ...
        wrapSize.y = (int) ((li + 1) * font.getLineHeight());
        breakLines = li;
    ...
    private int measureLine(int index, int start, int end, int line) {
        ...
        wrapSize.x = Math.max(lines[index][3], wrapSize.x);

        return len;
    }

    private int findILineByIndex(int index) {
    ...
    private int searchCharByX(int line, float x) {
        float w = 0;
    ...
    public void draw(Canvas canvas) {
        int y = Gravity.applyGravity(Gravity.vertical(gravity), viewport.y, Math.round(breakLines * font.getLineHeight()));
    ...
    private void drawCaret(Canvas canvas) {
        int l = selection.low();
        int line = findILineByIndex(selection.low());
    ...
    private void drawSelection(Canvas canvas) {
        int sline = findILineByIndex(selection.low());
        int eline = findILineByIndex(selection.high());
    ...
    private void drawSelection(Canvas canvas, int start, int end, int line) {
        int z = Gravity.applyGravity(Gravity.vertical(gravity), viewport.y, Math.round(breakLines * font.getLineHeight()));
        int w;
        ...
        int x = Gravity.applyGravity(Gravity.horizontal(gravity), viewport.x, w);
    ...
    public void drawLine(Canvas canvas, int y, int index) {
        ...
        int x = Gravity.applyGravity(Gravity.horizontal(gravity),viewport.x,line[3]);
